<?php 
    ...
    if(isset($_POST["submit"])) {
        ...
        for($i = 0; $i < count($subtopics); $i++) {
            $sql = mysqli_query($conn,"INSERT INTO subtopic_ranking (subtopic_id, rank)
                                        VALUES (". $subtopics[$i] .", ". $ranks[$i] .")
        ...
        header( 'location:' . $baseurl . 'controlgear/subtopicRanking?message=success' );
        exit();
    }

    if ( isset( $_GET[ 'message' ] ) && $_GET[ 'message' ] == 'success' ) {
        $errMsg = '<div class="alert alert-success" role="alert">Successfully Saved.</div>';
        echo "<script>window.history.pushState('','','" . $baseurl . "controlgear/subtopicRanking');</script>";
