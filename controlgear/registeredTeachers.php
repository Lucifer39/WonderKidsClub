...
$query = mysqli_query($conn, "SELECT id,userid,fullname,email,contact,status,school,created_at FROM users WHERE type=2");
...
if($query->num_rows > 0){
    ...
    //set column headers
    $fields = array('User ID','Name', 'Email', 'Contact','School','Status','Created Date');
  ...
  $schsql = mysqli_query($conn, "SELECT name FROM school_management WHERE id='".$row['school']."' and status=1");
	$schrow = mysqli_fetch_assoc($schsql);
        ...
        $lineData = array($row['userid'],$row['fullname'], $row['email'], $row['contact'],$schrow['name'],$status,$row['created_at']);
...
<div class="container-fluid">
	<div class="row">
		...
		<div class="col-md-12">
			<div class="grid bg-white box-shadow-light">
				<h5 class="heading d-flex align-items-center"><span class="mr-3">Teachers List</span> <form action="" method="post" enctype="multipart/form-data"><button type="submit" name="expsubmit" id="expsubmit" class="btn btn-primary custom-btn mb-0">Export</button></form></h5>
