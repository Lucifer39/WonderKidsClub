...
if ( isset( $_POST[ 'update' ] ) ) {
	...
	if ( !empty( $catid ) ) {		
		$resultset_1 = mysqli_query($conn, "select id from subject_class where name='".trim($catid)."'");
 ...
 if($count == 0)
	...
	mysqli_close( $conn );
	header( 'location:' . $baseurl . 'controlgear/subjectList' );
	exit;
...
if ( isset( $_POST[ 'submit' ] ) ) {
	$catid = mysqli_real_escape_string( $conn, $_POST[ 'name' ] );
	 ...
	 if ( !empty( $catid ) ) {		
		$resultset_1 = mysqli_query($conn, "select id from subject_class where name='".trim($catid)."' and type=1");
		$count = mysqli_num_rows($resultset_1);
 if($count == 0)
    {   

        mysqli_query( $conn, "INSERT INTO subject_class(name,slug,status,userid,type,created_at,updated_at) VALUES ('".trim($catid)."','".slugify(trim($catid))."',1,'".$sessionrow['id']."',1,NOW(),NOW())" );
		mysqli_close( $conn );
		$errMsg = '<div class="alert alert-success" role="alert">Successfully Saved</div>';
		header( 'location:' . $baseurl . 'controlgear/subjectList?message=success' );
		exit;
    }else{
       $errMsg = '<div class="alert alert-danger" role="alert">The "'.trim($catid).'" is already present.</div>';
    }	
...
if (isset($_POST["impsubmit"])) {
    ...
    if (in_array($_FILES["file"]["type"], $allowedFileType)) {
		...
		$file = $_FILES['file']['tmp_name'];
		  ...
		  $resultset_1 = mysqli_query($conn, "select id from subject_class where name='".trim($fname)."' and type=1");
		$count = mysqli_num_rows($resultset_1);
 if($count == 0)
    {	  
			  
          $sql = "insert into subject_class(name,status,userid,type,created_at,updated_at) values ('".$fname."','1','".$sessionrow['id']."',1,NOW(),NOW())";
         ...
         $c = $c + 1;
            ...
            if($sql){
             header( 'location:' . $baseurl . 'controlgear/subjectList?message=successimport' );
			 exit;
		} 
		 else
		 {
...
if ( isset( $_GET[ 'message' ] ) && $_GET[ 'message' ] == 'success' ) {
	$errMsg = '<div class="alert alert-success" role="alert">Successfully Saved.</div>';
	echo "<script>window.history.pushState('','','" . $baseurl . "controlgear/subjectList');</script>";
...
<div class="container-fluid">
	<div class="row">
		...
		<div class="col-md-12">
			<div class="grid bg-white box-shadow-light">
				<h5 class="heading">List of Subjects</h5>
