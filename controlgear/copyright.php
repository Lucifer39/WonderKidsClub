...
$sql = mysqli_query($conn, "SELECT id,body,meta_details FROM other_pages WHERE id=1");
...
if (isset($_POST['submit'])) {
	...
	$meta = mysqli_real_escape_string( $conn, $_POST[ 'metadata' ] );
	    ...
	    mysqli_query( $conn, "update other_pages Set body='".$bodyContent."',meta_details='".$meta."', updated_at=NOW() WHERE id=1" );		
		mysqli_close($conn);
		$errMsg = '<div class="alert alert-success" role="alert">Successfully Saved</div>';
		header( 'location:' . $baseurl . 'controlgear/copyright');
		exit();		
}
?>
