const botQuestions = [
  {
    id: 0,
    content: `Hello, ${current_student.name.split(" ")[0]}!`,
    type: "wait",
    input: "text",
  },
  {
...
} else if (student_progress?.bio !== null && adjective_array_php?.length == null) {
...
function addMessage(content, isBot) {
  ...
  if (isBot) {
    ...
    userProfile.src = `../type_master/assets/avatars/${
      current_student?.avatar || "default-icon.svg"
    }`;
        ...
        $.ajax({
          ...
          success: function (res) {
            ...
            response.forEach((element) => {
              ...
              buttonImage.src = `../type_master/assets/avatars/${element.filename}`;

              button.appendChild(buttonImage);

              button.addEventListener("click", () => {
                const messageImg = document.createElement("img");
                messageImg.src = `../type_master/assets/avatars/${element.filename}`;
...
function get_adjectives() {
  $.ajax({
    ...
    data: {
      alphabet: current_student.name.split(" ")[0][name_index],
    },
    success: function (res) {
      var response = JSON.parse(res);
...
function populate_button_input(adjectives) {
  if (
    name_index < current_student.name.split(" ")[0].length &&
    isNaN(current_student.name.split(" ")[0][name_index])
  ) {
    setTimeout(
      () =>
        addMessage(
          botQuestions[currentQuestionIndex].content.replace(
            "%%letter%%",
            current_student.name.split(" ")[0][name_index].toUpperCase()
          ),
          true
        ),
      500
    ...
    adjectives.forEach((element, index) => {
      ...
      if (!current_adjectives.includes(adjective[0].trim())) {
        ...
        buttonContent.addEventListener("click", () => {
          // var tooltip = bootstrap.Tooltip.getInstance(buttonContent);
          // if (tooltip) {
          //   tooltip.hide();
          // }
          ...
          if (currentState == "tooltip") {
            ...
            if (name_index < current_student.name.split(" ")[0].length) {
              ...
              $.ajax({
                ...
                success: function (res) {
                  ...
                  adjective_array.forEach((element, index) => {
                    if (!isNaN(current_student.name.split(" ")[0][index])) {
                      temp_message += current_student.name.split(" ")[0][offset_ind] + "<br>";
                      offset_ind++;
                    }

                    temp_message +=
                      current_student.name.split(" ")[0][offset_ind].toUpperCase() +
                      " - " +
                      element +
                      "<br>";
    ...
    if (name_index >= current_student.name.split(" ")[0].length) {
      $.ajax({
        ...
        success: function (res) {
          ...
          adjective_array.forEach((element, index) => {
            temp_message +=
              current_student.name.split(" ")[0][index].toUpperCase() + " - " + element + "<br>";
          });
...
function showProfile() {
  ...
  headerNameDiv.textContent = current_student.name;
  ...
  $.ajax({
    ...
    success: function (res1) {
      ...
      profilePic.src = `../type_master/assets/avatars/${response1.avatar}`;
      ...
      $.ajax({
        ...
        success: function (res) {
          ...
          const words = response.adjectives.split(",");
...
function incom_message(adj) {
  ...
  adj.forEach((element, index) => {
    ...
    temp_message +=
      current_student.name.split(" ")[0][offset_ind].toUpperCase() + " - " + element + "<br>";
