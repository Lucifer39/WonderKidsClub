...
<?php if(!empty($_POST["sbjName"])) {?>
			  ...
			  <?php $catsql = mysqli_query($conn, "SELECT a.topic,a.id FROM topics_subtopics as a INNER JOIN subject_class as b ON b.id=a.subject_id WHERE a.subject_id='".$_POST["sbjName"]."' and a.class_id='".$_POST["sbjclsName"]."' and parent=0 and a.status=1 and b.status=1");
							...
							<div class="form-row align-items-center form-group">
							  ...
							  <?php } else { 
									$chkPdfSql = mysqli_query($conn, "SELECT nm.filename, sc.pdf_class FROM nomenclature_mapping nm
																	JOIN subject_class sc ON sc.name = nm.class_name
																	WHERE subtopic_id = '". $subcatrow['id'] . "'");
									$countPdf = 0;

									while($chkPdfRow = mysqli_fetch_assoc($chkPdfSql)) {
										if(file_exists('../uploads/practice/'. $chkPdfRow['pdf_class'] .'/'. $chkPdfRow['filename'])) {
											$countPdf++;
										}
									}									
								?>
								...
								<input class="form-check-input hide" name="subtopic[]" type="checkbox" value="<?php echo $subcatrow['id']; ?>" id="flexCheckDefault_<?php echo $i; ?>" checked>
								<label class="form-check-label" for="flexCheckDefault_<?php echo $i; ?>">
								<?php echo $subcatrow['subtopic'] . " <span style='color: #006400'>(Pdf Generated: $countPdf)</span>"; ?>
								</label>
							  </div>
							  <button class="btn btn-primary custom-btn m-1" data-id="<?php echo $subcatrow['id']; ?>" onclick="runQuery()">Download</button>
							  <button class="btn btn-primary custom-btn m-1" data-id="<?php echo $subcatrow['id']; ?>" onclick="runDelete()">Delete PDFs</button>
							  </div>
							<?php } ?>
							<?php $i++; } ?>
							<?php } ?>
