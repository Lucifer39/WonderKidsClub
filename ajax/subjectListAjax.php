...
$sql = "SELECT id,name,status FROM subject_class WHERE type=1 order by id asc";
...
while( $employee = mysqli_fetch_assoc($resultset) ) {
	...
	$empRows[] = $status;
	$empRows[] = '<a href="javascript:void(0);" class="edit" data-toggle="modal" data-id="'.$employee['id'].'" data-name="'.$employee['name'].'" data-target="#editmodal"><i class="fa fa-edit tx-gray-light"></i></a>';
