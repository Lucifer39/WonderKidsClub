...
$sql = "SELECT id,subtopic,status,parent,userid,class_id,subject_id FROM topics_subtopics WHERE parent!=0 order by class_id,id asc";
...
while( $employee = mysqli_fetch_assoc($resultset) ) {
	...
	$tpcsql = mysqli_query($conn, "SELECT topic FROM topics_subtopics WHERE id='".$employee['parent']."' and status=1");
	$tpcrow = mysqli_fetch_assoc($tpcsql);
		
	$empRows = array();
	$empRows[] = $employee['id'];
	$empRows[] = $clsrow['name'].' / '.$subjrow['name'].' / '.$tpcrow['topic'];
	$empRows[] = $employee['subtopic'];
	$empRows[] = $status;
	$empRows[] = '<a href="javascript:void(0);" class="edit" data-toggle="modal" data-id="'.$employee['id'].'" data-name="'.$employee['subtopic'].'" data-target="#editmodal"><i class="fa fa-edit tx-gray-light"></i></a>';
